...
const {
    ...
    context,
    env
} = JSON.parse(fs.readFileSync('debug-status-event.json', {encoding: 'utf-8'}))
    ...
    try {
        ...
        try {
            ...
            if (responseBody.documents) {
                ...
                responseBody.documents.forEach((doc: any, index: number) => {
                    console.log(`  Document ${index + 1}:`);
                    console.log(`    ID: ${doc.documentId}`);
                    console.log(`    Status: ${doc.status}`);
                    console.log(`    Last updated: ${doc.lastUpdated}`);
                });
            } else if (responseBody.documentId) {
                console.log(`Single document status:`);
                console.log(`  ID: ${responseBody.documentId}`);
                console.log(`  Status: ${responseBody.status}`);
                console.log(`  Processing stage: ${responseBody.processingStage}`);
                console.log(`  Last updated: ${responseBody.lastUpdated}`);
            }
        } catch {
